...
const uint8_t contrastValues[4] = { 1, 5, 20, 40 };
uint8_t contrastIndex = 2;
...
void loop() {
    ...
    if (currentState == STATE_MENU_HOME) {
      if (click7) {
        ...
        if (menuIndex == 0) isLbs = !isLbs;
          ...
          contrastIndex = (contrastIndex + 1) % 4;
          setOledBrightness(contrastIndex);
        } else if (menuIndex == 4) {
  ...
  // BATTERY
  unsigned long batteryInterval = batteryInitialized ? 10000 : 30000;
  if (now - lastBatteryCheck >= batteryInterval) {
    lastBatteryCheck = now;
    float raw = analogRead(BATTERY_PIN);
    float currentV = (raw / 4095.0f) * 3.3f * (147.0f / 47.0f) * VOLTAGE_CORRECTION;
    smoothedBatteryV = currentV;
...
void loadSettings() {
  ...
  if (validMarker == 42) {
    ...
    contrastIndex = (storedContrast < 4) ? storedContrast : 2;
  }
}
